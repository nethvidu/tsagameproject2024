...
public class PlayerControllerRB2D : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        movingPlatformRelVel = Vector2.zero;
        Collider2D[] a = Physics2D.OverlapCircleAll((Vector2)transform.position - new Vector2(0, groundCheckOffset), groundCheckRadius, ground);
        a.ToList().ForEach(delegate (Collider2D a)
        {
            if (a.GetComponent<MovingMapObject>() != null)
            {
                onMovingPlatform = true;
                movingPlatformRelVel = a.gameObject.GetComponent<MovingMapObject>().Vel;
            }
        });
        if (onMovingPlatform)
        {
            transform.position += movingPlatformRelVel.ToVector3() * Time.fixedDeltaTime;
        }
        ...
        if(rb2D.velocity.y < 0.001f && !onMovingPlatform)
        {
            fall = true;
            jump = false;
        }
        ...
        isGrounded = Physics2D.OverlapCircle((Vector2)transform.position - new Vector2(0, groundCheckOffset), groundCheckRadius, ground) || Physics2D.OverlapCircle((Vector2)transform.position - new Vector2(0, groundCheckOffset), groundCheckRadius, ground);
